...
if (strlen($_SESSION['sturecmsaid']==0)) {
 ...
 $notmsg=$_POST['notmsg'];
 $foruser="users";
$sql="insert into tblpublicnotice(NoticeTitle,NoticeMessage,Foruser)values(?,?,?)";
$query=$dbh->prepare($sql);
 $query->execute(array($nottitle, $notmsg,$foruser));
   $LastInsertId=$dbh->lastInsertId();
   if ($LastInsertId>0) {
    echo '<script>alert("Notice has been added.")</script>';
echo "<script>window.location.href ='add-notice-team.php'</script>";
  }
  else
    {
